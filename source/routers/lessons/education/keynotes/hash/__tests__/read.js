...
describe('keynote read by hash', () => {
    ...
    test('should return 200 for getting keynote by hash', async (done) => {
        const response = await server.get('/lessons/:lessonHash/keynotes/:keynoteHash');

        expect(response.statusCode).toBe(200);
        done();
    });

    test('should return an object type for getting keynote by hash', async (done) => {
        const response = await server.get('/lessons/:lessonHash/keynotes/:keynoteHash');
