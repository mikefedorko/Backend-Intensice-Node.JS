...
describe('video create', () => {
    ...
    test('should return 201 for post video', async (done) => {
        const response = await server.post('/lessons/:lessonHash/videos');

        expect(response.statusCode).toBe(201);
        done();
    });

    test('should return an object type for posting video', async (done) => {
        const response = await server.post('/lessons/:lessonHash/videos');
