...
describe('video read by hash', () => {
    ...
    test('should return 200 for getting video by hash', async (done) => {
        const response = await server.get('/lessons/:lessonHash/videos/:videoHash');

        expect(response.statusCode).toBe(200);
        done();
    });

    test('should return an object type for getting video by hash', async (done) => {
        const response = await server.get('/lessons/:lessonHash/videos/:videoHash');
