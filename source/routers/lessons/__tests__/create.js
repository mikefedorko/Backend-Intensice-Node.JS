...
describe('lesson create', () => {
    ...
    test('should return 201 for post lesson', async (done) => {
        const response = await server.post('/lessons');

        expect(response.statusCode).toBe(201);
        done();
    });

    test('should return an object type for posting lesson', async (done) => {
        const response = await server.post('/lessons');
