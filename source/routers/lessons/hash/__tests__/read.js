...
describe('lesson read by hash', () => {
    ...
    test('should return 200 for getting lesson by hash', async (done) => {
        const response = await server.get('/lessons/:lessonHash');

        expect(response.statusCode).toBe(200);
        done();
    });

    test('should return an object type for getting lesson by hash', async (done) => {
        const response = await server.get('/lessons/:lessonHash');
