...
describe('user update by hash', () => {
    ...
    test('should return 200 for updating user by hash', async (done) => {
        const response = await server.put('/users/:userHash');

        expect(response.statusCode).toBe(200);
        done();
    });

    test('should return an object type for updating user by hash', async (done) => {
        const response = await server.put('/users/:userHash');
