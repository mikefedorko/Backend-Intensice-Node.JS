...
describe('user read by hash', () => {
    ...
    test('should return 200 for getting user by hash', async (done) => {
        const response = await server.get('/users/:userHash');

        expect(response.statusCode).toBe(200);
        done();
    });

    test('should return an object type for getting user by hash', async (done) => {
        const response = await server.get('/users/:userHash');
